  ...
  return createPortal(
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/55 p-3 backdrop-blur-sm sm:p-5" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
      <div ref={dialogRef} tabIndex={-1} role="dialog" aria-modal="true" aria-labelledby="add-provider-title" aria-describedby="add-provider-description" className="flex max-h-[calc(100dvh-1.5rem)] w-full max-w-2xl flex-col overflow-hidden rounded-2xl border border-line bg-bg shadow-2xl outline-none sm:max-h-[calc(100dvh-2.5rem)]">
        <div className="flex shrink-0 items-start justify-between gap-4 border-b border-line bg-bg/95 px-5 py-4 backdrop-blur-xl sm:px-6">
          <div>
            ...
            <p id="add-provider-description" className="muted mt-1 text-xs">Choose a provider and give this route a recognizable name.</p>
          </div>
        ...
        <form onSubmit={submit} className="flex min-h-0 flex-1 flex-col">
          <div className="dashboard-scroll min-h-0 flex-1 overflow-y-auto px-5 py-5 sm:px-6 sm:py-6">
            <fieldset>
            ...
            <div className="mt-6 grid gap-4 sm:grid-cols-2">
              <label className="block">
                ...
                <input value={name} onChange={(event) => setName(event.target.value)} placeholder="e.g. Production OpenAI" className="input" />
              </label>
              <label className="block">
            ...
            <div className="mt-5 flex flex-col gap-3 rounded-xl border border-gold/20 bg-gold-soft/45 p-3.5 sm:flex-row sm:items-center sm:justify-between">
            ...
            {testState === 'success' && <p role="status" className="mt-4 flex items-center gap-2 text-xs text-success"><CheckCircle2 className="h-4 w-4" aria-hidden="true" />Connection looks good. Ready to save.</p>}
            {error && <p role="alert" className="mt-4 flex items-center gap-2 text-xs text-danger"><CircleAlert className="h-4 w-4" aria-hidden="true" />{error}</p>}
          </div>

          <div className="flex shrink-0 flex-col-reverse gap-2 border-t border-line bg-bg/95 px-5 py-4 backdrop-blur-xl sm:flex-row sm:justify-end sm:px-6">
